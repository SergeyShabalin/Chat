...
export default function Messages() {
    const {allMessages, messageEndRef} = useContext(Context)
    ...
    const messages = allMessages.map(message => {
        ...
        return (
            <Stack key={id} spacing={1}>
                <Typography textAlign="left" pl='0.5rem' color='#06063f'
                            fontSize='0.7rem'>{user === message.name ? 'Вы' : message.name}</Typography>
    ...
    return (
        <Stack maxHeight='75%' padding='5px' overflow="auto">{messages}
